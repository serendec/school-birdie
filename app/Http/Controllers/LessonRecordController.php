...
class LessonRecordController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return $this->respond([
                ...
                'redirect_url' => route('lesson_record.create'),
            ], $request);
        }
        ...
        return $this->respond([
            ...
            'redirect_url' => route('lesson_record.index')
        ], $request);
    }
    ...
    public function show(Request $request)
    {
        ...
        $videos = explode(',', $lessonRecord->video);

        // 動画お気に入り情報を取得
        $favorites = Favorite::where('user_id', Auth::id())
                                ->where('video_category', 'lesson_record')
                                ->where('video_category_id', $lessonRecord->id)
                                ->pluck('video')
                                ->toArray();
        ...
        return view('lesson_record.show', compact('lessonRecord', 'previousLessonRecord', 'nextLessonRecord', 'totalRecordsNumber', 'currentRecordNumber', 'msg', 'videos', 'favorites'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            return $this->respond([
                'result' => 'error',
                'msg'    => __('messages.error'),
                'redirect_url' => route('lesson_record.show', $request->id),
            ], $request);
        }
        ...
        return $this->respond([
            ...
            'redirect_url' => route('lesson_record.show', $lessonRecord->id),
        ], $request);
    }
