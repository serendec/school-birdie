...
@section('content')
    <div class="site-body-wrapper">
        <h1 class="hd-1">編集</h1>
        <div class="detail">
            <form method="post" action="{{ route('tag.update') }}" class="formlist">
                @csrf
                @method('PATCH')
                        ...
                        <div class="detail-content-body-item ma-0">
                            <div class="inputset">
                                ...
                                <div class="text size-middle block">
                                    <input type="text" name="name" id="input-name" value="{{ old('name', $tag->name) }}" size="40" required>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <form id="delete-form" action="{{ route('tag.delete') }}" method="POST">
                @csrf
                @method('DELETE')
                <input type="hidden" name="id" value="{{ $tag->id }}">
            </form>
